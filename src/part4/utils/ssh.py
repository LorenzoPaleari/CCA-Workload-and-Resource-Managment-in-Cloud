...
def ssh_exec_out(name, command):
    name_checked = _check_name(name)
    result = subprocess.check_output(f"gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing-24 ubuntu@{name_checked} --zone europe-west3-a --command ".split() + [command])
    return result.decode('utf-8')

def ssh_exec_out_no_check(name, command):
    name_checked = _check_name(name)
    os.system(f"gcloud compute ssh --ssh-key-file ~/.ssh/cloud-computing-24 ubuntu@{name_checked} --zone europe-west3-a --command '{command}'")
