...
import sys
import subprocess
from utils.memcached import get_memcached_status, install_memcached
from utils.mcperf import install_mcperf, launch_mcperf
from utils.ssh import ssh_exec, ssh_exec_out

_cluster_name = "part4.k8s.local"
_machine_names = {}
_machine_names_ext = {}
...
def _get_nodes_data():
    ...
    ip_ext_index = column_names.index('EXTERNAL-IP')
    nodes_ext = {}
    for line in lines[1:]:
        if line:
            columns = line.split()
            name = columns[name_index]
            ip = columns[ip_ext_index]
            nodes_ext[name] = ip
    global _machine_names_ext
    _machine_names_ext = {k[:-5]: {"name": k, "ip": v} for k, v in nodes_ext.items()}
...
def get_machine_names_ext() -> dict:
    if len(_machine_names_ext.items()) == 0:
        _get_nodes_data()
    return _machine_names_ext
