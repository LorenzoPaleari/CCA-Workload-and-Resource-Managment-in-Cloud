...
# Read the CSV data
df_0 = pd.read_csv('../part3/1/jobs_time.csv')
df_1 = pd.read_csv('../part3/1/memcache_cores.csv')
df_2 = pd.read_csv('../part3/1/p95.csv')
...
# Set Title
title = ax.set_title('1A', fontproperties=font, fontsize=16, fontweight='bold', pad=12, ha='center')
...
ax.plot(df_2['time'], df_2['p95'], color='blue', linewidth=1.5, marker="o", markersize=8, label='95th Latency',
        markeredgecolor='white', zorder=9, clip_on=False)
...
for i in range (7):
    ax.axhline(y=3.6 - i / 5, color='black', linestyle='-', linewidth=0.5, zorder=1)
...
for label in labels:
        ...
        for i in range(0, len(df_0[label].values), 2):
                if df_0[label].values[i] >= 0:
                        last = i+1
                        ax.hlines(xmin=df_0[label].values[i] - 0.15, xmax=df_0[label].values[i+1] + 0.15, y=3.6 - index / 5, color=colors[index], linewidth=15, zorder=10)
        ax.plot([df_0[label].values[0], df_0[label].values[0]], [0, 3.6 - index / 5], color=colors[index], linewidth=2, linestyle='--', zorder=6, clip_on=False)
        ax.plot([df_0[label].values[last], df_0[label].values[last]], [0, 3.6 - index / 5], color=colors[index], linewidth=2, linestyle='--', zorder=6)
        index += 1
...
ax.set_ylim(0, 4)  # y limits from 0 to 1
ticks = [0,0.5,1,1.5,2,2.4,2.6,2.8,3,3.2,3.4,3.6]
ticks_label = ["0.0", "0.5", "1.0", "1.5", "2.0", "vips", "radix", "freqmine", "ferret", "dedup", "canneal", "blackscholes"]
ax.set_yticks(ticks)
ax.set_yticklabels(ticks_label)
ax.yaxis.set_label_coords(-0.026, 0.525)
...
ax2.legend(loc='upper right', bbox_to_anchor=(0.83, 1), facecolor='white')
...
# Show the plot
plt.show()
fig.savefig('../Plot_images/plot3_1A.svg', format='svg')
